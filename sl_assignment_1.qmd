...
The shape of the distribution of median house values is fairly bell-shaped, with 41% (208) of median house values falling between \~\$18K and \$23K.\
While 93% (471) of observations belong to the category with chas=0, the median house value for observations with chas=1 is 11% higher at \$23,3K compared to those in the chas=0 category. Additionally, the observations in the chas=1 group show a skew towards higher values, unlike the chas=0 group, which has a more balanced distribution around the median. Despite these differences in distribution, the two box plots overlap, suggesting that the difference in means between the two groups may not be immediately statistically significant (a difference-of-means hypothesis test would have to be conducted to formalize this).
...
Grouped by the **chas** feature, for cases where chas =1 (which have shown to have higher median house values) median house values correlations lose their statistical significance with 7 features, namely **crime, indus, age ,dis, rad, tax, and ptratio.\
**\
...
The out-of-sample (OOO) MSE for the baseline model is at \$16.64K, that is, on average the baseline model's home values predictions are expected to deviate from the actual by this value (which is greater that the standard deviation of median house values of \~\$9.2K).
...
3.  All regressors also maintain their sign with very little changes to their marginal effects with respect to the response variable.

4.  R square (\~74%) and the F statistic (significant at 1%) are still in the same region as the baseline model.
...
1.  The best regularization parameters for the Ridge and LASSO models are reached at at $\lambda$ =0.69 (CV_MSE~min~ of \~25.48) and 0.69 ( CV_MSE~min~ \~27.9), respectively . The 1~se~ equivalent parameters for Ridge and LASSO are obtained at $\lambda$=4.86 (CV_MSE~min~ \~28.79) and $\lambda$=0.67 (CV_MSE~min~ \~27.99).
...
Method: The **chas** and **rad**[^1] categorical variable provides an easy way of segmenting the data into distinct groups for which interactions with other variables can be analyzed. Thus the following process is used to choose the interaction feature:

[^1]: Measured as an index as per meta data definition
...
3.  Analyze the correlations of all these interaction variables wth respect to the response variable, and choose the interaction with the highest, and most statistically significant correlation.

4.  Add this term to the best-performing model, which in this case was the **full variable regression.**

5.  Again, for the sake of data completeness, the interaction correlation analysis is done on the full data set before addition to the model.
...
#| fig-height: 12
#| fig-width: 14
#| fig-cap: "Interaction Features Pairplot"
...
Across both pair plots, the lstat_rad interaction feature show the highest (in abolute value) and most statistically significant correlation (at 1%) amongst interacted features. This will be added to the training data for the best performing model.
...
for(i in 2:length(vars_final)){
  ...
  p <- ggplot(x_df, aes(x = .data[[var_name]], color = factor(chas))) +
    ...
    labs(
      title = paste("Median House Value vs", var_name),
      x = var_name,
      y = "medv"
    ) +
    theme_minimal()
...
Despite the significance of the two individual features, lstat and rad, the engineered interaction feature did not prove to be a statistically significant predictor of median house values \* talk about potential multicollinearity indications
...
2.  Homoscedasticity: The scale-location plot does not indicate that the variance of the residuals is constant[^2], rather it seems to follow an increasing trend when plotted against fitted values. This also greatly inhibit the ability to make valid statistical inferences, especially regarding the variance and precision of the regressors as well as the overall validity (significance) of the model.

[^2]: Homoscedasticity requires the plotted points to be scattered randomly about the line, with no discernible pattern.
