...
1.  The best regularization parametrs for the Ridge and LASSO models are reached at at $\lambda$ =0.69 (CV_MSE~min~ of \~25.48) and 0.69 ( CV_MSE~min~ \~27.9), respectively . The 1~se~ equivalent parameters for Ridge and LASSO are obtained at $\lambda$=4.86 (CV_MSE~min~ \~28.79) and $\lambda$=0.67 (CV_MSE~min~ \~27.99).
...
3.  The **nox** feature still maintains its position as the largest absolute marginal predictor of median house values across three out of the four models, with the rm feature having the largest marginal contribution to response variable.
4.  In addition to a change in magnitude, some feature coefficients also show a change in sign in some of models: the **indus** coefficient also sees a change (positive to negative) in sign from the baseline in both the Ridge~1SE~ and Ridge~min~ models while the **rad** feature coefficient changes sign in the Ridge~1SE~ model (although this feature coefficient is highly penalized relative to the baseline).

**Predictions On Test Data**

For both the Ridge and LASSO regressions, the 1SE models will be used to favour model simplicity and reduce the risk of over fitting; these are used to predict on the test data.
...
test_mses_df=data.frame(metric=c('MSE'),
                        all_features=test_mses[1],
                        backwise_step=test_mses[2],
                        ridge=test_mses[3],
                        lasso=test_mses[4],
                        `average across models`=mean(test_mses)
                        )
...
Although the test scores MSE are relatively close to each other for 3 out of the 4 models, the variable selection models fail to outperform the baseline model LASSO having the worst test mse out of the 4. \* Talk about implications of multicollinearity in conclusion
...
All the model predictions show relatively good fit to the general shape of the response variable in the test data.

**Inclusion of Interaction Term**

Method: The **chas** feature which is a categorical variable provides an easy way of segmenting the data into two distinct groups for which partial regression curves for the individual features can be visually inspected for signs of interaction. Thus the following process is used to choose the interaction feature:

1.   Create interaction terms for all features with the chas feature: $\text{Feature}_i \times \text{chas}$

2.  Analyze the correlations of all these interction variables with response variable, medv. Choose the interaction with the highest, and statistically significant correlation.
