...
![](CV_MSE_PLOT.png){width="614" height="292"}

```{r,echo=F,eval=F}

min_lam_index=which(cv_model_ridge$lambda==cv_model_ridge$lambda.min)
cv_model_ridge$cvm[min_lam_index]

se_lambda_index=which(cv_model_ridge$lambda==cv_model_ridge$lambda.1se)
cv_model_ridge$cvm[se_lambda_index]
```
...
knitr::kable(ridge_coeff, 
col.names = c(
    "Feature",
    "$\\lambda_{min}$",
    "$\\lambda_{1se}$"
  ),
escape = FALSE,caption = 'Ridge Regression Coefficients')
...
# 
# mean(train_pred_lambda_min)
# 
# y=boston_data_training$medv
# 
# sst_min=sum((y - mean(y))^2)
# sse_min=sum((train_pred_lambda_min - y)^2)
# 
# rsq_min=1-(sst_min/sse_min)
# 
# sst_1se=sum((y - mean(y))^2)
# sse_1se=sum((train_pred_lambda_1se - y)^2)
# 
# rsq_1se=1-(sst_1se/sse_1se)
# rsq_min;rsq_1se
```
...
Method: Same as ridge regression, use cross validation to find the optimal regularization parameter(s) ,lambda, that produce the minimum CV MSE error during training and compare the coefficients produced by the model.
...
![LASSO CV MSE vs Regularization Parameter (lambda)](lasso_cv_mse.png){width="533" height="218"}
...
knitr::kable(lass_coeff, 
col.names = c(
    "Feature",
    "$\\lambda_{min}$",
    "$\\lambda_{1se}$"
  ),
escape = FALSE,caption = 'LASSO Regression Coefficients')
```
